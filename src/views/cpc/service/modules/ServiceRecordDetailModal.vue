<template>
  <a-form :form="form" class="content">
    ...
    <a-card title="订单信息" class="card">
      <a slot="extra" href="#"></a>
      <div class="grid-cell double">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="服务标题" class="ant-col-sm-12">
          <a-input v-decorator="[ 'name']" :readOnly="disableSubmit" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="申请人" class="ant-col-sm-12">
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="备注" class="ant-col-sm-12">
          <a-input v-decorator="[ 'nation', {}]" :readOnly="disableSubmit" />
        </a-form-item>
...
export default {
  ...
  computed: {
    badgeStyle() {
      const Text = {0: '已完成', 1: '待受理', 2: '进行中'}
      const Status = {0: 'success', 1: 'processing', 2: 'warning'}
      return {
  ...
  mounted () {
    const { id, serviceStatus } = this.$route.query
