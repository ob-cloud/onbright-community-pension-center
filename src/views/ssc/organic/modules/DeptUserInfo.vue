...
<script>
  ...
  import { getDepartUserList, unbindDepartUser, editDepartUser } from '@/api/ssc'
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          list: "/depart/getUserList",
          edit: "/depart/editDepartWithUser",
    ...
    methods: {
      ...
      loadData(arg) {
        ...
        getDepartUserList(params).then((res) => {
      ...
      batchDel: function () {
        ...
        if (this.selectedRowKeys.length <= 0) {
          ...
          this.$confirm({
            ...
            onOk: function () {
              unbindDepartUser({depId: that.currentDeptId, userIds: ids}).then((res) => {
                if (this.$isAjaxSuccess(res.code)) {
      ...
      handleDelete: function (id) {
        ...
        var that = this;
        // deleteAction(that.url.delete, {depId: this.currentDeptId, userId: id})
        unbindDepartUser({depId: this.currentDeptId, userId: id}).then((res) => {
          if (this.$isAjaxSuccess(res.code)) {
            that.$message.success("删除用户与选中部门关系成功！");
            if (this.selectedRowKeys.length>0){
      ...
      selectOK(data) {
        ...
        console.log(params);
        editDepartUser(params).then((res) => {
          if (this.$isAjaxSuccess(res.code)) {
