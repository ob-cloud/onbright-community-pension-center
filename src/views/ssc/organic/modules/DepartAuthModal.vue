...
<script>
  ...
  import {saveDepartPermission, queryDepartPermission} from '@/api/ssc'
  import { queryPermissionTreeList } from '@/api/system'
    ...
    methods: {
      ...
      loadData(){
        queryPermissionTreeList().then((res) => {
          ...
          queryDepartPermission({depId: this.departId}).then((res)=>{
            if (!this.$isAjaxSuccess(res.code)) return
