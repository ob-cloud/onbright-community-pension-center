...
<script>
  ...
  import { getDepartRoleList } from '@/api/ssc'
  ...
  export default {
    ...
    methods: {
      ...
      loadData(arg) {
        ...
        getDepartRoleList(params).then((res) => {
      ...
      handleEdit (record) {
        ...
        this.$refs.modalForm.edit(record, record.depId)
      },
      handleAdd: function () {
        if (!this.currentDeptId) {
          this.$message.error('请选择一个部门!')
      ...
      handlePerssion (record) {
        this.$refs.modalDeptRole.show(record.id, record.depId)
      },
    }
  }
</script>
